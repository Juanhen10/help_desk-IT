...
import os
from user_window import Ui_MainWindow as UserWindow
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.loginPin.setEchoMode(QLineEdit.Password) # deixa a senha criptografada
        ...
        self.pinBTN.clicked.connect(self.check_pin)
    ...
    def open_user_window(self, pin):
        self.user_window = QMainWindow()
        self.ui = UserWindow()
        self.ui.setupUi(self.user_window)  # Configura a interface do usuário
        self.ui.load_user_data(pin)  # Carrega os dados do usuário
    ...
    def validate_pin(self, pin):
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(1) FROM users WHERE pin = ?", (pin,))
    ...
    def open_user_window(self, pin):
        ...
        self.ui.load_user_data(pin)
        self.user_window.show()
    
    def show_error_message(self):
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Icon.Critical)
        msg.setText("PIN inválido")
